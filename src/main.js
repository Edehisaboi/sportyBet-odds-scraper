  ...
  const output = {
    ...
    unmatched: rows
      ...
      .map((row) => ({
        ...
        nearest: row.nearest_event_id ? {
          eventId: row.nearest_event_id,
          fixture: `${row.nearest_home_team} vs ${row.nearest_away_team}`,
          kickoff: row.nearest_kickoff_utc,
          homeScore: row.nearest_home_score,
          awayScore: row.nearest_away_score,
          weakestScore: row.nearest_weakest_score,
          combinedScore: row.nearest_combined_score,
        } : null,
      })),
    totalQuotes: rows.reduce((total, row) => total + row.quote_count, 0),
    marketCoverage: summarizeCoverage(rows),
    indexedEvents: index.events.length,
    indexReportedTotal: index.totalNum,
    indexPasses: index.passes,
